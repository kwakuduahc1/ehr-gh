...
namespace ShimsServer.Repositories
{
    ...
    public class PatientSchemesRepository(IConnection connection) : IPatientSchemesRepository
    {
        ...
        {
            const string sql = """
                    INSERT INTO patientschemes (patientschemesid, patientsid, schemesid, cardid, isactive, expirydate, username)
                    VALUES (@PatientSchemesID, @PatientsID, @SchemesID, @CardID, @IsActive, @ExpiryDate, @UserName);
                    """;
            using var con = await connection.ConnectionAsync(cancellationToken);
