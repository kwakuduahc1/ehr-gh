...
export class PrintProviderService {
  ...
  print(selector: string, title: string = "Payments Manager", outer: boolean = true, css: boolean = true, style: boolean = true, delay: number = 500, tag: boolean = false, canvas: boolean = false) {
    let table = $(`#${selector}`);
    table.printThis({
      ...
      header: `<br /> <br /><h1 class="text-center text-tertiary">${title}</h1>`,               // prefix to html
      footer: `<footer>bStudio</footer>`,               // postfix to html
